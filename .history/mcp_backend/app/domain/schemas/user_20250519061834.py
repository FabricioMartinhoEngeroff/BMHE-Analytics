...
class UserRequest(BaseModel):
    ...
    email: constr(regex=r"(^[\w\.-]+@[\w\.-]+\.\w{2,}$)") = Field(
        ..., 
        description="Email deve ser válido"
    )

    password: constr(min_length=8, regex=r"^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[\W_]).+$") = Field(
        ..., 
        description="Senha forte com letra maiúscula, minúscula, número e caracteres especiais"
    )

    cpf: constr(regex=r"^\d{3}\.\d{3}\.\d{3}-\d{2}$") = Field(
        ..., 
        description="CPF no formato xxx.xxx.xxx-xx"
    )

    telefone: constr(regex=r"^\(\d{2}\)\s?\d{4,5}-\d{4}$") = Field(
        ..., 
