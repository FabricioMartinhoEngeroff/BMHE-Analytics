...
class UserService:
    def __init__(self, user_repo: UserRepository, password_encoder: PasswordEncoder, Whatsaap_service: WhatsAppService):
        self.user_repo = user_repo
        self.password_encoder = password_encoder
        self.Whatsaap_service = Whatsaap_service
    ...
    def create_user(self, db: Session, data: UserRequest) -> UserDTO:
        ...
        if self.user_repo.exists_by_email(db, data.email):
            raise DuplicateResourceException("email", "A user with this email already exists.")

        if self.user_repo.exists_by_cpf(db, data.cpf):
        ...
        self.Whatsaap_service.send_welcome_message(user.name, user.telefone)
    ...
    def _update_user_fields(self, user: User, dto: UserRequest):
        ...
        if dto.endereco:
    user.endereco.rua    = dto.endereco.rua
    user.endereco.numero = dto.endereco.numero
    user.endereco.bairro = dto.endereco.bairro
    user.endereco.cidade = dto.endereco.cidade
    user.endereco.estado = dto.endereco.estado.value
    user.endereco.cep    = dto.endereco.cep
