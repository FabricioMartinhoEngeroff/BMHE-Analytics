...
class UserService:
    ...
    def create_user(self, db: Session, dto: UserRequest) -> UserDTO:
        ...
        if self.user_repo.exist_by_email(db, dto.email):
            raise DuplicateResourceException("email", "A user with this email already exists.")
        if self.user_repo.exist_by_cpf(db, dto.cpf):
        ...
        user = User(
            ...
            endereco=dto.endereco.dict(),  # assuma que sua coluna é JSON ou dict
        )
    ...
    def _update_user_fields(self, user: User, dto: UserRequest) -> None:
        # atualiza somente se vier no DTO
        if dto.name:
            user.name = dto.name
        if dto.email:
